...
save_dir = f'experiment_results/{settings['run_name']}'
...
save(settings,save_dir + "/settings")
...
plt.subplot(1,3,3)
plt.title('f(x)')
plt.tricontourf(fine_grid[:,0],fine_grid[:,1],jax.vmap(rhs_functions[0])(fine_grid),100)
plt.colorbar()

plt.savefig(save_dir+'/example_a_u.png')
plt.close()
...
plt.tight_layout()
plt.savefig(save_dir+'/u_results.png')
plt.close()
...
save(result_dict,save_dir + '/error_results')
...
pd.DataFrame(result_summary_dict).to_csv(save_dir + '/error_summary.csv')
print(f"Time spent on run: {time.time() - start_time}")
