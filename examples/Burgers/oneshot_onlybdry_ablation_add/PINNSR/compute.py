...
from KernelTools import eval_k,dx_k,dxx_k,dt_k,dtx_k,dtt_k
...
def get_u_pde_adj(u0_new):
    ...
    class PdeModel():
        ...
        def get_grid_features(u_params):
            ...
            U_tx = grid_features[:,3]
            U_tt = grid_features[:,4]

            UU = jnp.multiply(U,U)
            UU_x = jnp.multiply(U,U_x)
            UU_xx = jnp.multiply(U,U_xx)
            UU_tx = jnp.multiply(U,U_tx)
            UU_tt = jnp.multiply(U,U_tt)

            U_xU_x = jnp.multiply(U_x,U_x)
            U_xU_xx = jnp.multiply(U_x,U_xx)
            U_xU_tx = jnp.multiply(U_x,U_tx)
            U_xU_tt = jnp.multiply(U_x,U_tt)

            U_xxU_xx = jnp.multiply(U_xx,U_xx)
            U_xxU_tx = jnp.multiply(U_xx,U_tx)
            U_xxU_tt = jnp.multiply(U_xx,U_tt)

            U_txU_tx = jnp.multiply(U_tx,U_tx)
            U_txU_tt = jnp.multiply(U_tx,U_tt)

            U_ttU_tt = jnp.multiply(U_tt,U_tt)


            full_features = jnp.vstack([CONST,U,U_x,U_xx,U_tx,U_tt,
                        UU,UU_x,UU_xx,UU_tx,UU_tt,
                        U_xU_x,U_xU_xx,U_xU_tx,U_xU_tt,
                        U_xxU_xx,U_xxU_tx,U_xxU_tt,
                        U_txU_tx,U_txU_tt,
                        U_ttU_tt]).T
