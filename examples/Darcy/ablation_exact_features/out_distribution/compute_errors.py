...
def run_exp_o_dis_err(m,obs_pts,run):
    ...
    feature_operators = tuple([diff_x_op,diff_xx_op,diff_y_op,diff_yy_op])
...
# Single run at fixed (m, obs_pts) -- ablation study, not a sweep
M_FUN = 32
OBS_PTS = 2
RUN = 0

res = run_exp_o_dis_err(M_FUN, OBS_PTS, RUN)
err = {'1_5_mthd': float(res[0]), '2_mthd': float(res[1])}
jnp.save('errors', err)
print('1_5_mthd (1-step):', res[0])
print('2_mthd (2-step):', res[1])
print('success!')
