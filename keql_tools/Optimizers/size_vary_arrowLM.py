...
def setup_arrow_functions(model,beta_reg_u,beta_reg_P,datafit_weight):
    ...
    data_args = [model.collocation_points,model.rhs_forcing_values,model.observation_points,model.observation_values]
        ...
        residuals = (
            jnp.hstack(
                [
                    ...
                    u_param,P_params,
                    colloc,rhs,obs_points,obs_values
                    ) for u_param,colloc,rhs,obs_points,obs_values in zip(u_params,*data_args)
                ]
            )
        )
        return 0.5 * (
    ...
    single_jacU = jax.jacrev(single_function_residuals,argnums = 0)
    # _jacU_func = jax.vmap(jax.jacrev(single_function_residuals,argnums = 0),in_axes=u_vmap_axes)
    def _jacU_func(u_params,P_params,data_args = data_args):
        return jnp.vstack(
                [
                    single_jacU(
                    u_param,P_params,
    ...
    single_jacP = jax.jacrev(single_function_residuals,argnums = 1)
    def _jacP_func(u_params,P_params,data_args = data_args):
        return jnp.vstack(
                [
                    single_jacP(
                    u_param,P_params,
